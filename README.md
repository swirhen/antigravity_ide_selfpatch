...
チャットペイン最上部に、折りたたみ式のセッション一覧ドロワーを常設します（バーには「セッションリストを開く」とシンプルに表示され、ヘッダーのタイトルと重複しません）。

- **現在セッション最上位ソート**: 現在開いているセッションが、更新時刻やピン留め状態にかかわらず**常にリストの一番上（最上位）に固定表示**されます。
- **視覚的インジケーター**: 現在開いているセッションの左端に**青いラジオアイコン**を表示。ピン留め状態は右側のピン留めボタン（金色ピンアイコン）に集約し、左右重複のないすっきりとしたUIに統一。
...
チャットペイン最上部の**セッションタイトルをクリック**するか、チャット欄で `F2` を押すか、`:rename <新しい名前>`（短縮 `:ren`）を入力して Enter を押すだけで、AI への余計な API リクエストを発生させずに**瞬時にセッション名を永続変更**できます。
...
### 5. ⚙️ Antigravity チャットペインパッチ設定ダイアログ（GUI 設定画面）

ドロワーバー右端の**歯車アイコン（チャットペインパッチ設定ボタン）**をクリックすると、独自の設定ダイアログが開きます。

- **「チャットをEnterで送信しない」トグルスイッチ**:
  Enter改行 / Ctrl+Enter送信（ON）と、通常のチャット挙動（OFF: Enter送信 / Shift+Enter改行）を、滑らかなスライド式トグルスイッチ（Switch Slider）でワンクリック切り替え可能。
- **チャットコマンドのカスタマイズ**:
  各機能の通常コマンド名（例: `:resume`）および短縮コマンド名（例: `:res`）を自由に変更できます。
- **キーボードショートカット設定（keybindings.json を直接編集）**:
  Keybindings GUI画面のフリーズを回避するため、ダイアログ内の「`keybindings.json を開く`」ボタンから直接設定ファイルを安全に開けます。ダイアログ内にはパッチで追加された全コマンド（`antigravity.session.*`）と規定キー、機能内容が一覧表示されます。
- **保存 ＆ ワンクリックリロード**:
