...
"""
================================================================================
Antigravity IDE - チャットペイン機能拡張パッチ 自動適用スクリプト
================================================================================
このスクリプトは、Antigravity IDE のチャットペインに対して以下の機能をワンクリックで安全に適用します：
  ...
  6. チャット欄右下にAIクォータ利用状況・残量バー・リセット時間表示

ロジック本体は独立モジュール `antigravity-session-patch.js` で管理され、
本スクリプトが本体（workbench.desktop.main.js）へビルド時バンドル・フック適用します。
（Electron Sandbox や CSP の制約を回避し、確実に動作します）
"""
